...
3. Authentication is required but token is invalid`);
    ...
    throw new Error(
      `Failed to call SiYuan API: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
    ...
    // 文档管理
    {
      ...
      inputSchema: {
        ...
        properties: {
          notebook: {
            type: "string",
            description: "笔记本 ID（可选，不提供则使用当前笔记本）",
          },
          path: {
            type: "string",
            description: "文档路径，如 /daily/2025-08-03",
          },
    ...
    // 块操作
    {
      ...
      inputSchema: {
        ...
        properties: {
          dataType: {
            type: "string",
            description: "数据类型 (markdown 或 dom)",
            default: "markdown",
          },
  ...
  try {
    ...
    switch (name) {
          ...
          if (data.code === 0 && data.data && data.data.notebooks) {
            return {
              content: [
                {
                  type: "text",
                  text: JSON.stringify(data.data.notebooks, null, 2),
                },
              ],
            };
          } else {
      ...
      case "open_notebook": {
        ...
        const result = await api("/api/notebook/openNotebook", {
          notebook: args.notebook,
        });
        if (result.code === 0) {
          return {
            content: [
              { type: "text", text: `✅ 笔记本打开成功: ${args.notebook}` },
            ],
          };
        } else {
          return {
            content: [{ type: "text", text: `❌ 打开失败: ${result.msg}` }],
          };
        }
      }
      ...
      case "close_notebook": {
        ...
        const result = await api("/api/notebook/closeNotebook", {
          notebook: args.notebook,
        });
        if (result.code === 0) {
          return {
            content: [
              { type: "text", text: `✅ 笔记本关闭成功: ${args.notebook}` },
            ],
          };
        } else {
          return {
            content: [{ type: "text", text: `❌ 关闭失败: ${result.msg}` }],
          };
        }
      }
      ...
      case "rename_notebook": {
        ...
        if (result.code === 0) {
          return {
            content: [
              { type: "text", text: `✅ 笔记本重命名成功: ${args.name}` },
            ],
          };
        } else {
          return {
            content: [{ type: "text", text: `❌ 重命名失败: ${result.msg}` }],
          };
        }
      }
      ...
      case "create_notebook": {
        ...
        const result = await api("/api/notebook/createNotebook", {
          name: args.name,
        });
        if (result.code === 0) {
          return {
            content: [
              { type: "text", text: `✅ 笔记本创建成功: ${args.name}` },
            ],
          };
        } else {
          return {
            content: [{ type: "text", text: `❌ 创建失败: ${result.msg}` }],
          };
        }
      }
      ...
      case "remove_notebook": {
        ...
        const result = await api("/api/notebook/removeNotebook", {
          notebook: args.notebook,
        });
        if (result.code === 0) {
          return {
            content: [
              { type: "text", text: `✅ 笔记本删除成功: ${args.notebook}` },
            ],
          };
        } else {
          return {
            content: [{ type: "text", text: `❌ 删除失败: ${result.msg}` }],
          };
        }
      }
      ...
      case "get_notebook_conf": {
        ...
        const result = await api("/api/notebook/getNotebookConf", {
          notebook: args.notebook,
        });
        return {
      ...
      case "create_doc": {
        ...
        if (result.code === 0) {
          return {
            content: [{ type: "text", text: `✅ 文档创建成功: ${args.path}` }],
          };
        } else {
          return {
            content: [{ type: "text", text: `❌ 创建失败: ${result.msg}` }],
      ...
      case "rename_doc": {
        ...
        if (result.code === 0) {
          return {
            content: [
              { type: "text", text: `✅ 文档重命名成功: ${args.title}` },
            ],
          };
        } else {
          return {
      ...
      case "remove_doc": {
        ...
        if (result.code === 0) {
          return {
            content: [{ type: "text", text: `✅ 文档删除成功: ${args.path}` }],
          };
        } else {
          return {
            content: [{ type: "text", text: `❌ 删除失败: ${result.msg}` }],
      ...
      case "remove_doc_by_id": {
        ...
        const result = await api("/api/filetree/removeDocByID", {
          id: args.id,
        });
        if (result.code === 0) {
          return {
            content: [{ type: "text", text: `✅ 文档删除成功: ${args.id}` }],
          };
        } else {
          return {
            content: [{ type: "text", text: `❌ 删除失败: ${result.msg}` }],
      ...
      case "move_docs": {
        ...
        if (result.code === 0) {
          ...
          return {
            content: [{ type: "text", text: `❌ 移动失败: ${result.msg}` }],
          };
        }
      }
      ...
      case "get_block_kramdown": {
        ...
        const result = await api("/api/block/getBlockKramdown", {
          id: args.id,
        });
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
      ...
      case "export_md_content": {
        ...
        const result = await api("/api/export/exportMdContent", {
          id: args.id,
        });
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
      ...
      case "check_siyuan_status": {
        ...
        try {
          ...
          status.errors.push(
            `版本检查失败: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
        ...
        try {
          ...
          status.errors.push(
            `笔记本列表检查失败: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
        ...
        try {
          ...
          status.errors.push(
            `SQL 查询检查失败: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
...
⚙️ 环境变量:
${Object.entries(info.environment)
