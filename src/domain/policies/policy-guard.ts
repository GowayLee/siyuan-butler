...
export function buildWritePlanReviewChecks(plan: WritePlan): ReviewCheck[] {
  const hasSideEffectRisk = plan.side_effects.some(
    (effect) => effect.kind !== "none",
  );
  const autoRepairableCaptureWrite = isAutoRepairableCaptureWrite(plan);
  ...
  return [
    ...
    },
    {
      ...
      result: !canWritePlanEnterReview(plan)
        ? "fail"
        : (hasSideEffectRisk && !autoRepairableCaptureWrite) ||
            plan.needs_confirmation === true
          ? "warn"
          : "pass",
      note: !canWritePlanEnterReview(plan)
        ? "仍有未收拢的问题，暂不进入执行。"
        : autoRepairableCaptureWrite
          ? "当前只会补建缺失的 Sparkles 标题并落下这条 Sparkle，仍属于可控低风险写入。"
          : hasSideEffectRisk
            ? "本次写入带有附带影响，建议先确认范围。"
            : plan.needs_confirmation === true
              ? "当前计划建议先做一次用户确认。"
...
export function isAutoRepairableCaptureWrite(plan: WritePlan): boolean {
  return (
    plan.origin === "capture" &&
    plan.operation_type === "append-sparkle" &&
    plan.risk_level === "low" &&
    plan.side_effects.length === 1 &&
    plan.side_effects[0]?.kind === "multi-block-write" &&
    plan.target_section.section_kind === "sparkles" &&
    plan.target_section.section_id === undefined &&
    plan.target_page.page_id !== undefined
  );
}
...
export function writePlanNeedsExplicitConfirmation(plan: WritePlan): boolean {
  return (
    ...
    (plan.side_effects.some((effect) => effect.kind !== "none") &&
      !isAutoRepairableCaptureWrite(plan))
  );
}

export function deriveReviewDecisionFromWritePlan(
  plan: WritePlan,
): ReviewDecision {
...
export function createReviewResultFromWritePlan(plan: WritePlan): ReviewResult {
  ...
  if (decision === "allow") {
    const allowReason = isAutoRepairableCaptureWrite(normalizedPlan)
      ? "写入目标清晰，预览完整；当前只会先补建缺失的 Sparkles 标题，再在其下落下这条 Sparkle。"
      : "写入目标清晰，预览完整，且当前计划没有额外副作用需要确认。";
    const allowSummary = isAutoRepairableCaptureWrite(normalizedPlan)
      ? "这次写入边界已经收拢清楚，可以在补齐 Sparkles 标题后直接继续。"
      : "这次写入边界已经收拢清楚，可以直接继续。";

    return {
      decision,
      reason: allowReason,
      review_summary: allowSummary,
  ...
  if (decision === "ask_confirm") {
    return {
      ...
      user_prompt:
        "我已经把这次写入范围收拢好了。若按这个计划继续，就只会执行这里列出的内容。",
      final_write_plan: normalizedPlan,
